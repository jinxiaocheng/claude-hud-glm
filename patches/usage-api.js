...
function readClaudeSettings() {
    try {
        ...
        debug('读取 settings.json 失败:', error);
        return null;
    }
}

// 检查是否配置了 GLM (z.ai 或 bigmodel.cn)
function isGlmConfigured(settings) {
    const baseUrl = settings?.env?.ANTHROPIC_BASE_URL;
    if (!baseUrl) return false;
    return baseUrl.includes('z.ai') || baseUrl.includes('bigmodel.cn');
}

// 调用 GLM Coding Plan 配额查询接口
// 端点: /api/monitor/usage/quota/limit
// Authorization 直接使用 token，不加 Bearer 前缀
function fetchGlmQuotaApi(apiKey, baseUrl) {
    return new Promise((resolve) => {
        ...
        if (baseUrl.includes('api.z.ai')) {
            ...
            try {
                quotaHost = new URL(baseUrl).hostname;
            }
            catch {
                quotaHost = 'open.bigmodel.cn';
            }
        }
        else {
            debug('无法识别的 GLM base URL:', baseUrl);
            resolve(null);
            return;
        }
        const options = {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                if (res.statusCode !== 200) {
                    debug('GLM 配额 API 返回非200状态:', res.statusCode);
                    resolve(null);
                    return;
                }
                try {
                    ...
                    debug('解析 GLM 配额 API 响应失败:', error);
                    resolve(null);
                }
            });
        });
        req.on('error', (error) => {
            debug('GLM 配额 API 请求错误:', error);
            resolve(null);
        });
        req.on('timeout', () => {
            debug('GLM 配额 API 请求超时');
            req.destroy();
            resolve(null);
        });
        req.end();
...
// GLM 缓存 TTL
const GLM_CACHE_TTL_MS = 60_000;
const GLM_CACHE_FAILURE_TTL_MS = 15_000;

// 获取 GLM Coding Plan 配额用量
// 复用原版缓存机制，返回与 Anthropic 相同的 UsageData 结构
async function getGlmUsage(homeDir, now, settings) {
    // 检查缓存
    const cacheState = readCacheState(homeDir, now, { cacheTtlMs: GLM_CACHE_TTL_MS, failureCacheTtlMs: GLM_CACHE_FAILURE_TTL_MS });
    ...
    try {
        const apiResponse = await fetchGlmQuotaApi(apiKey, baseUrl);
        if (!apiResponse) {
            const failureResult = {
                planName: 'GLM',
                fiveHour: null,
        ...
        for (const item of limits) {
            if (item.type === 'TOKENS_LIMIT' && item.percentage != null) {
                if (item.unit === 3) {
                    fiveHourPercent = Math.max(0, Math.min(100, Math.round(item.percentage)));
                    if (item.nextResetTime) {
                        fiveHourResetAt = new Date(item.nextResetTime);
                    }
                }
                else if (item.unit === 6) {
                    sevenDayPercent = Math.max(0, Math.min(100, Math.round(item.percentage)));
                    if (item.nextResetTime) {
        ...
        debug('GLM 配额获取失败:', error);
        return null;
    }
}
// ========== GLM Coding Plan 配额支持结束 ==========
...
export async function getUsage(overrides = {}) {
    ...
    // GLM Coding Plan 配额检测：如果配置了 z.ai 或 bigmodel.cn，使用 GLM 配额 API
    const claudeSettings = readClaudeSettings();
    if (isGlmConfigured(claudeSettings)) {
        return getGlmUsage(homeDir, now, claudeSettings);
    }
